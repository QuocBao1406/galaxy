...
function display_comments($parent_id, $comments_by_parent, $post_id, $loggedIn, $conn)
{
    ...
    foreach ($comments_by_parent[$parent_id] as $comment) {
        ...
        if (!empty($comment['content']))
            echo '<p class="mb-0 small text-light">' . nl2br(htmlspecialchars($comment['content'])) . '</p>';
        ...
        if ($result_media->num_rows > 0) {
            ...
            while ($media = $result_media->fetch_assoc()) {
                if ($media['media_type'] == 'image')
                    echo '<img src="' . htmlspecialchars($media['file_path']) . '" class="comment-media">';
                elseif ($media['media_type'] == 'video')
                    echo '<video src="' . htmlspecialchars($media['file_path']) . '" controls class="comment-media"></video>';
            }
            echo '</div>';
        }
        $stmt_media->close();
        ...
        if ($loggedIn) {
            ...
            echo '  <form action="/galaxy/components/submit_comment.php" method="POST" enctype="multipart/form-data" class="comment-submission-form">';
...
<body>
    ...
    <div class="main-body">
        <main class="container">
            <a href="/galaxy/pages/congdong.php" class="btn btn-outline-secondary mb-4"><i
                    class="fas fa-arrow-left"></i> <?= t('congdong-12') ?> </a>

            <div class="post">
                <div class="post-header">
                    <img src="<?php echo !empty($post['avatar']) ? htmlspecialchars($post['avatar']) : '/galaxy/images-icon/default_avatar.png'; ?>"
                        class="avatar">
                ...
                if ($result_media_post->num_rows > 0) {
                    ...
                    while ($media = $result_media_post->fetch_assoc()) {
                        if ($media['media_type'] == 'image')
                            echo '<img src="' . htmlspecialchars($media['file_path']) . '">';
                        elseif ($media['media_type'] == 'video')
            ...
            <div class="comment-section" data-text-comment-plural="<?= htmlspecialchars(t('congdong-11')) ?>"
                data-text-error-generic="<?= htmlspecialchars(t('error-generic')) ?>">

                <h3 class="text-white mb-3" id="comment-count"><?php echo $total_comments; ?>
                    <?= htmlspecialchars(t('congdong-11')) ?></h3>
                <?php if ($loggedIn): ?>
                    <div class="comment-form">
                        <form action="/galaxy/components/submit_comment.php" method="POST" enctype="multipart/form-data"
                            class="comment-submission-form">
                            <input type="hidden" name="post_id" value="<?php echo $post_id; ?>">
                            <div class="comment-input-area">
                                <textarea name="content" rows="1"
                                    placeholder="<?= htmlspecialchars(t('congdong-13')) ?>"></textarea>
                                <label class="file-upload-label" title="Thêm media"><i class="fas fa-paperclip"></i></label>
                                <input type="file" name="media[]" multiple class="comment-media-input"
                                    accept="image/*,video/*" style="display:none;">
                                <button type="submit" class="btn-send-comment" title="Gửi"><i
